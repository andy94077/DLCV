...
cv2.imwrite('3-2_x.png', np.where(img_x > 4, 0, 255).astype(np.uint8))
cv2.imwrite('3-2_y.png', np.where(img_y > 4, 0, 255).astype(np.uint8))

# (3)
grad_img = np.sqrt(img_x ** 2 + img_y ** 2)
grad_blur = np.sqrt(cv2.filter2D(blur, -1, kx)** 2 + cv2.filter2D(blur, -1, ky)** 2)

cv2.imwrite('3-3_original.png', np.where(grad_img > 6, 0, 255).astype(np.uint8))
cv2.imwrite('3-3_blur.png', np.where(grad_blur > 6, 0, 255).astype(np.uint8))
